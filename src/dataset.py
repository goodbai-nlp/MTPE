    ...
    def setup(self, stage='fit'):
        ...
        if self.line_by_line:
            ...
            def tokenize_function(examples):
                ...
                tgt = examples['tgt']
                pe = examples['pe']
                src = [self.prefix +  inp + " </s>" + " zh_CN" for inp in src]
                tgt = [inp + " </s>" + " en_XX" for inp in tgt]
                src = [s + t for s,t in zip(src, tgt)]
                src_inputs = self.tokenizer(src, add_special_tokens=False, max_length=self.max_src_length, padding=padding, truncation=True)
                # tgt = self.tokenizer(tgt, max_length=self.max_src_length, padding=padding, truncation=True)
                # Setup the tokenizer for targets
                pe = [inp + " </s>" +" en_XX" for inp in pe]
                labels = self.tokenizer(pe, add_special_tokens=False, max_length=self.max_tgt_length, padding=padding, truncation=True)
